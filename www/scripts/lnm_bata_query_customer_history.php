...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Lipa Na M-PESA at Bata, Interaction History</p>
        <br />
        <form name='bata_query_customer_history' action='lnm_bata_query_customer_history.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("vw_blnm_staging");
				$columns = array("msisdn","to_char(trx_date,'DD-MON-YYYY')","mpesa_trx_id","TRANSACTION_AMOUNT","Bata_Store");
                $titles = array("msisdn", "TrxDate","Mpesa Trx","Amount","Bata Till Store");
                $advanced = "where msisdn = '".'254'.substr($_POST['msisdn'],-9)."' order by trx_date desc";
				$display_str = $db->display_records($tables, $columns, $titles, "bata_query_customer_history", $advanced, null, null, "");
