<?php
    ...
    $BONGA_DATABASEUSER = "furahi_checker";
    $BONGA_DATABASEPASSWORD = "furahi321";
    ...
    if (isset($_POST['msisdn'])) {
		...
		if ($error == "") {
			...
			if ($error == "") {
				...
				$query = "BEGIN TOPUP_FIESTA.RESEND_AIRTIME_PIN(:IN_MSISDN, :IN_SERIAL_NUMBER, :OUT_MESSAGE); END;";
				$cursor = ocinewcursor($db->link);
				$stmt = ociparse ($db->link, $query);                                //bind variables
				$in_msisdn = $msisdn;
				$in_sr_number = "CM: ".$_POST['sr_number'];
				$out_message = "Message queued for submisssion. Kindly confirm receipt with winner";
	
				logmessage("DEBUG", "Resend furahi airtime: ".$in_msisdn."; $serial_number; ".$_POST['sr_number']."; ".$_SESSION['USERID']);
...
<div class="cspacer">
	...
	<div align="center" style="text-align:left; width:100%">
		...
		<form name='acc_history_list' action='resend_furahi_airtime.php' method='post'>
		<center>
