...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Query SubReg Whitelist</p>
        <br />
        <form name='query_subreg_whitelist' action='query_subreg_whitelist.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("tbl_mpesa_whitelist");
				$columns = array("msisdn","trx_date","agent_name","agent_location","additional_info");
                $titles = array("Msisdn", "TrxDate","Agent Name","Agent Location","Comments");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by tid desc";
				$display_str = $db->display_records($tables, $columns, $titles, "query_subreg_whitelist", $advanced, null, null, "");
