<?php
    ...
    $BUNDLES_DATABASEUSER = "prepaidblackberry";
    $BUNDLES_DATABASEPASSWORD = "ppblack123";
...
<div class="cspacer">
        ...
        <form name='acc_history_list' action='check_blackberry_subscriptions.php' method='post'>
        <center>
        ...
        ?>
                        <?php
                                ...
                                $tables = array("account_history a");
								
                            
								$columns = array("msisdn", "action", "no_of_days","to_char(from_date, 'dd-MON-yyyy hh24:mi')", "to_char(to_date, 'dd-MON-yyyy')","to_char(request_date, 'dd-MON-yyyy hh24:mi')","to_char(processed_date, 'dd-MON-yyyy hh24:mi')");
   
								
								$titles = array("msisdn", "request type", "period", "start date", "end date", "request date", "processed date");
                
				
								$advanced = "where msisdn = ".substr($_POST['msisdn'], -9)." order by id desc";
								
								$display_str = $db->display_records($tables, $columns, $titles, "acc_history_list", $advanced, null, null,  "id");
