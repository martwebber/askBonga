...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Query Data Award Status</p>
        <br />
        <form name='coke_query_data_per_coke' action='coke_query_data_per_coke.php' method='post'>
        <center>
...
$tables = array("tbl_coke15_awards");
				$columns = array("msisdn","icode_id","award_status","award_details","to_char(award_date,'DD-Mon-YYYY HH24:MI')","subscriber_type");
                $titles = array("msisdn", "iCode","Status","Details","Date","SubType");
                $advanced = "where icode_id = upper(DBMS_OBFUSCATION_TOOLKIT.md5 (input => UTL_RAW.cast_to_raw(upper('".$_POST['icode']."'))))";
				$display_str = $db->display_records($tables, $columns, $titles, "coke_query_data_per_coke", $advanced, null, null, "");
