...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Query SIMSwap Whitelist</p>
        <br />
        <form name='query_simswap_whitelist' action='query_simswap_whitelist.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("tbl_dealer_whitelist");
				$columns = array("msisdn","trx_date","dealer_name","dealer_location","additional_info");
                $titles = array("Msisdn", "TrxDate","Dealer Name","Dealer Location","Comments");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by tid desc";
				$display_str = $db->display_records($tables, $columns, $titles, "query_simswap_whitelist", $advanced, null, null, "");
