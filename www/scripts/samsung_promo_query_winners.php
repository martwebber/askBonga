...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Ultimate Upgrade Promo Winner Details</p>
        <br />
        <form name='samsung_promo_query_winners' action='samsung_promo_query_winners.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("vw_winners_full");
				$columns = array("msisdn","to_char(date_of_win,'DD-MON-YYYY HH24:MI')","confirm_winner","to_char(draw_date,'DD-MON-YYYY HH24:MI')","prize_name");
                $titles = array("msisdn", "Win Date","Confirm Status","Draw Date","Prize");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by tid desc";
				$display_str = $db->display_records($tables, $columns, $titles, "samsung_promo_query_winners", $advanced, null, null, "");
