<?php
    ...
    $BONGA_DATABASEHOST = "10.65.12.245";
    $BONGA_DATABASEPORT = "1521";
    $BONGA_DATABASEUSER = "iebc";
    $BONGA_DATABASEPASSWORD = "Iebc#1234";
    $BONGA_DATABASENAME = "iebc";

    if (isset($_POST['msisdn'])) {
        $validator =  new Validate();
	$msisdn = substr(trim($_POST['msisdn']),-9);
        if (!$validator->is_valid_msisdn($msisdn)) {
		...
		if ($error == "") {
			...
			if ($error == "") {
				...
				$query = "BEGIN CM_OPT_OUT(:IN_MSISDN, :IN_SR_NUM, :IN_CM_USER, :OUT_MESSAGE); END;";
				$cursor = ocinewcursor($db->link);
				$stmt = ociparse ($db->link, $query);                                //bind variables
				$in_msisdn = $msisdn;
				$in_sr_number = $_POST['sr_number'];
				$out_message = "Successfully submitted, message sent to sub";
	
				logmessage("DEBUG", "IEBC opt out: ".$in_msisdn."; $in_sr_number; ".$_SESSION['USERID']);
				ocibindbyname($stmt, ":IN_MSISDN", &$in_msisdn, -1);
				ocibindbyname($stmt, ":IN_SR_NUM", &$in_sr_number, -1);
				ocibindbyname($stmt, ":IN_CM_USER", &$_SESSION['USERID'], -1);
				...
				if ($result) {
					...
					$e = oci_error($stmt);
					logmessage("ERROR", "IEBC: ".$e['message']);
				}
...
<div class="cspacer">
	...
	<div align="center" style="text-align:left; width:100%">
		...
		<form name='acc_history_list' action='iebc_opt_out.php' method='post'>
		<center>
		<table class="tablebody border" width="80%">
			...
			<tr>
				<td><br />SR_NUMBER:</td><td><br /><input type='text' name='sr_number' /></td>
			</tr>                        
			<tr>
