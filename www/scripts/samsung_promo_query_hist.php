...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Ultimate Upgrade Promo History Details</p>
        <br />
        <form name='samsung_promo_query_hist' action='samsung_promo_query_hist.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("tbl_ssp_request_hist");
				$columns = array("msisdn","to_char(trx_date,'DD-MON-YYYY HH24:MI')","event","event_details");
                $titles = array("msisdn", "TrxDate","Event","Details");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by tid desc";
				$display_str = $db->display_records($tables, $columns, $titles, "samsung_promo_query_hist", $advanced, null, null, "");
