...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Query DND  Details</p>
        <br />
        <form name='query_broadcast_opt_out' action='query_broadcast_opt_out.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("tbl_bulk_sms_opt_out");
				$columns = array("msisdn","date_opted_out");
                $titles = array("Msisdn", "TrxDate");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by id desc";
				$display_str = $db->display_records($tables, $columns, $titles, "query_broadcast_opt_out", $advanced, null, null, "");
