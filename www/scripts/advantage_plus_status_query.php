...
//                    echo "Connected";
                    ...
                    if ($msisdn != '') {
                        $sql = "SELECT PP_ID, PP_MPESA_TRANSACTION_ID, PP_OMPUBLIC_ID, PP_ORDER_NUMBER, PP_MSISDN, PP_DEPOSIT_AMOUNT, PP_PROCESS_STATUS, PP_REQUEST_DATE, PP_ORDER_TYPE, PP_ORDER_PROFILE,PP_ERROR_MSG FROM PP_PRE2HYB_REQUESTS WHERE  PP_MSISDN = '$msisdn' ORDER BY PP_REQUEST_DATE DESC ";
                    } else {
                        $sql = "SELECT PP_ID, PP_MPESA_TRANSACTION_ID, PP_OMPUBLIC_ID, PP_ORDER_NUMBER,PP_MSISDN, PP_DEPOSIT_AMOUNT, PP_PROCESS_STATUS, PP_REQUEST_DATE, PP_ORDER_TYPE, PP_ORDER_PROFILE,PP_ERROR_MSG FROM PP_PRE2HYB_REQUESTS WHERE  PP_PROCESS_STATUS = '$process_status' OR PP_MSISDN = '$msisdn' ORDER BY PP_REQUEST_DATE DESC";
                    }
                    ...
                    while ($row = oci_fetch_array($stid, OCI_RETURN_NULLS + OCI_ASSOC)) {
						...
						$pp_trxid = $row['PP_MPESA_TRANSACTION_ID'];
                        ...
                        if ($process_status == "ERROR") {
                            echo '<td>';
                            echo ' ';
                            echo '<a class="btn btn-danger" href="resubmit.php?PP_ID=' . $row['PP_ID'] . ' &PP_PROCESS_STATUS= ' . $row['PP_PROCESS_STATUS'] . '&USER_ID='.$SESSION_ID.'">Resubmit</a>';
                            echo '</td>';
                        }
                        echo "</tr>";
                        // Close the table
