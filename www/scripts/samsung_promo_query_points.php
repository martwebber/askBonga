...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Ultimate Upgrade Promo Query Points</p>
        <br />
        <form name='samsung_promo_query_points' action='samsung_promo_query_points.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("tbl_ssp_summary_pts");
				$columns = array("msisdn","to_char(date_created,'DD-MON-YYYY HH24:MI')","to_char(last_updated,'DD-MON-YYYY HH24:MI')","points");
                $titles = array("msisdn", "TrxDate","Last Update","Points");
                $advanced = "where substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by tid desc";
				$display_str = $db->display_records($tables, $columns, $titles, "samsung_promo_query_points", $advanced, null, null, "");
