...
<br />
        <p style="font:Arial, Helvetica, sans-serif; font-size:12px; color:#FF3300; padding: 5px;">Bonga PIN Reset SMS Sent Details</p>
        <br />
        <form name='lms_sms_status' action='lms_query_sms_status.php' method='post'>
        <center>
        ...
        ?>
			...
			<?php
$tables = array("SENT_SMS_T");
				$columns = array("msisdn","to_char(date_queued,'DD-MON-YYYY HH24:MI')","decode(sms_submitted,'1','Yes','0','No','Unknown')","substr(MESSAGE,0,22)||'XXXX'||substr(MESSAGE,27,73)");
                $titles = array("msisdn", "TrxDate","SMS Sent","Message");
                $advanced = "where sender = 'Bonga' and substr(msisdn,-9) = '".substr($_POST['msisdn'],-9)."' order by id desc";
				$display_str = $db->display_records($tables, $columns, $titles, "lms_sms_status", $advanced, null, null, "");
