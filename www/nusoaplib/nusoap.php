...
class nusoap_base {
	...
	* @access   public
	*/
    var $soap_defencoding = 'ISO-8859-7';
			...
			if ($pos_xml !== FALSE) {
				...
				if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res)) {
					...
					if (strtoupper($xml_encoding) != $encoding) {
						...
						$this->debug($err);
						if ($encoding != 'ISO-8859-1' || strtoupper($xml_encoding) != 'UTF-8' || $encoding != 'ISO-8859-7') {
							$this->setError($err);
							return;
						}
...
class nusoapclient extends nusoap_base  {
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->xml_encoding = 'ISO-8859-7'; // fallback to default HTTP encoding.
		$this->debug('Use encoding: ' . $this->xml_encoding . ' when creating soap_parser');
